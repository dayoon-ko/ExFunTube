...
class MobileNetV3(nn.Module):
    ...
    def _forward_impl(self, x_org: Tensor) -> (Tensor, Tensor):
        xs, featuress = [], []
        for i in range(0, x_org.shape[-1], 100):
            x = self.features(x_org[:,:,:,i:i+100])
            features = F.adaptive_avg_pool2d(x, (1, 1)).squeeze()
            x = self.classifier(x).squeeze()
            if features.dim() == 1 and x.dim() == 1:
                # squeezed batch dimension
...
class EnsemblerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        all_out = [i / len(self.models) for i in all_out]

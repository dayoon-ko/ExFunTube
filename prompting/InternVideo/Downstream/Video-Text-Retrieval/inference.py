...
def eval_epoch(args, model, test_dataloader, device, n_gpu, store=True):
    ...
    with torch.no_grad():
        for bid, batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
            ...
            # load video segments
            segment_path = args.video_dir + '/' + vid + '/' + args.segmentation_filename
            with open(segment_path) as f:
                segments = json.load(f)
                if 'vcap' in segments[0]:
                    continue
            ...
            for b in batch:
                ...
                for i in range(0, len(input_ids), 10):
                    sequence_output_sub, visual_output_sub = model.get_sequence_visual_output(
                        torch.cat(input_ids[i:i+10]), torch.cat(segment_ids[i:i+10]), torch.cat(input_mask[i:i+10]), 
                        video[i:i+10].unsqueeze(1), video_mask[i:i+10].unsqueeze(1)
                    )
                ...
                visual_output = torch.cat(visual_output)
                
                cum = 0
                for i in input_ids:
                    l = len(i)
                    batch_sequence_output_list.append(sequence_output[cum : cum+l].squeeze())
                    cum += l
                for i in range(len(input_mask)):
                    batch_list_t.append((input_mask[i], segment_ids[i],))
                    batch_visual_output_list.append(visual_output[i])#.unsqueeze(0))
                    batch_list_v.append((video_mask[i],))
                ...
                for i in range(len(input_ids)):   
                    sim_matrix = _run_on_single_gpu(model, [batch_list_t[i]], [batch_list_v[i]], [batch_sequence_output_list[i]], [batch_visual_output_list[i]])
                    idxs.append(np.argmax(sim_matrix[0].squeeze()))
                output.append([sentences[i][idx] for i, idx in enumerate(idxs)])
